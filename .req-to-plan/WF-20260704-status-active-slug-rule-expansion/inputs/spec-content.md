...
parseRequirements(text, S):
  for raw in text.split(/\r?\n/):
    ...
    if /^[a-z][a-z0-9+.-]*:\/\//i.test(tr) || tr.includes('://'): continue   // URL 行
    addName(tr, S)
```
...
### SPEC-SYNCTEST-001 检测用例 (a)–(g)（DES-TEST-001 [ADDRESSED]）
...
- (d) `writeF(repo/'pyproject.toml','[tool.poetry.dependencies]\npython = "^3.11"\nfastapi = "^0.110"\n')` → `ids.includes('fastapi')` 且 `!includes python 相关误取`。
- (e) `writeF(repo/'pyproject.toml','[project.optional-dependencies]\ndev = ["pytest"]\n')` → `ids.includes('pytest')`。
- (f) 畸形 pyproject（如 `writeF(repo/'pyproject.toml','[[[bad')`）+ 有效 `writeF(repo/'package.json', JSON({dependencies:{vue:"^3"}}))` → 不抛、`ids.includes('vue')`。
...
- npm Trusted Publishing（docs.npmjs.com/trusted-publishers）：OIDC 需 npm ≥ 11.5.1 且 Node ≥ 22.14.0；`permissions.id-token: write`+`contents: read`；`setup-node` `registry-url`；免长期 token。（Context7 `/websites/npmjs` 已核。）
- PEP 621（`[project].dependencies` / `optional-dependencies`）、PEP 735（`[dependency-groups]`、`include-group`）、PEP 503（名字归一）、Poetry `[tool.poetry(.group.<g>).dependencies]` 表键约定 —— 均反映在 SPEC-PYDEPS-001。
- SPEC-RULE-001/AUDIT-001 的官方文档核查在执行期逐条进行（Expo、.NET MAUI、SolidJS、Blazor、Electron 安全清单、Prisma、web.dev CWV、OWASP LLM Top 10、Next.js16/Tailwind v4/Svelte 5/Swift 6）。
...
| 同上 | (d) poetry 表 | fastapi 检出、python 剔除 |
