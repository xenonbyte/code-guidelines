...
Verification: (1) 目标：`node -e "const y=require('fs').readFileSync('.github/workflows/ci.yml','utf8'); if(!/fail-fast:\s*false/.test(y)||!/windows-latest/.test(y)) process.exit(1)"` 或用可得的 YAML linter 验证语法与矩阵；(2) 证据：粘贴文件内容并核对与 SPEC 逐字一致。G1（PR 上 9 腿绿）在合入 PR 后于 CI 实测。
...
Verification: (1) 目标：`git diff AGENTS.md CLAUDE.md` 仅新增一句；`node --test test/readme.test.mjs`（若涉及）与全量 `node --test` 绿；(2) 证据：粘贴 diff 与测试输出。
...
Skeleton:
```js
function mergePythonDeps(pyprojectTexts, requirementsTexts) { /* SPEC-PYDEPS-001 状态机 */ }
...
```
Steps:
- [ ] 先写 `test/sync.test.mjs` 的 (a)–(g) 七例与 `test/stacks.test.mjs` 的 `pythonDeps` validator 分支（红）。
- [ ] `scanRepo`：`EXCLUDED_DIRS` 加 `.venv`/`venv`/`__pycache__`；收集 `pyprojectTexts`/`requirementsTexts`（SPEC-SCAN-001）。
- [ ] 实现 `mergePythonDeps`（SPEC-PYDEPS-001：单/跨行数组、poetry 表键剔 `python`、`include-group` 内联表剥离、requirements 剥注释+仅跳整行 URL、PEP503 归一）。
...
- [ ] 全部转绿。
Verification: (1) 目标：`node --test test/sync.test.mjs test/stacks.test.mjs` 全绿（尤其 (a) 误报回归）；(2) 全量 `node --test` + `npm run check` 绿；(3) scratch 仓 dogfood 三例（main.py-only 不检出 fastapi/security、pyproject+fastapi 检出、requirements+flask 检出）；(4) 证据：粘贴测试通过计数与 dogfood 输出。
...
Verification: (1) 目标：全量 `node --test`（含 library.test/stacks.test 的 57 计数与 readme.test 双语对齐）+ `npm run check` 绿；(2) scratch 仓 dogfood：9 个新 stack 逐一命中（expo/MauiProgram.cs/.razor/solid-js/zustand/electron/prisma/openai + 纯前端命中 web-perf）、12 条上限截断且截断文件在报告具名；(3) 证据：粘贴测试计数与 dogfood 命中/截断输出。
...
TDD Applicable: no
Files:
- `assets/library/{nextjs,tailwind,svelte,ios-swiftui}.md`（必查）及其余九条如有过时（react/vue/nuxt/angular/astro/html-css/react-native/flutter/android-compose）
Skeleton:
```text
...
新增纯函数，契约按下列伪码（不引 TOML 库；每个文本独立 `try/catch` 静默跳过；PEP503 归一）：
```
...
```
边界契约：poetry `python = "^3.11"` 被 `key != 'python'` 剔除；poetry 内联表依赖 `fastapi = { version="^0.1" }` 由表键模式取 `fastapi`；extras（`uvicorn[standard]`）由名字正则截断为 `uvicorn`；`[project]` 的 `keywords`/`classifiers`/`requires-python` 等非 `dependencies` 键跳过；PEP 735 `include-group` 内联表经 `stripInlineTables` 移除。
...
- `mergePythonDeps(pyprojectTexts:string[], requirementsTexts:string[]) -> Set<string>`（PEP503 归一名；纯函数；异常静默）。
...
| PEP 621 / 735 / 503 + Poetry 表键约定 | PEP 508/503 | 2026-07-04 | dependencies / optional-dependencies / dependency-groups(+include-group) / poetry 表键 → PEP503 归一，反映于 SPEC-PYDEPS-001 |
