...
r2p_version: 5
r2p_status: approved
r2p_created_at: 2026-07-04T06:37:12.802569+00:00
r2p_updated_at: 2026-07-04T07:03:53.112267+00:00
---
...
- **requirements 行解析**：先剥离行内注释（`\s+#…`），再跳过空行、整行 `#` 注释、`-` 开头选项行、以及**整行以 URL scheme 开头**的直链行（不用 `includes('://')`，避免误伤带内联 URL 注释或 `pkg @ url` 的行）；其余取行首 `^[A-Za-z0-9][A-Za-z0-9._-]*`。
